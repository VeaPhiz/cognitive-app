...
const getScoreRating = (level) => {
  if (level > 9) return { label: "Superhuman",    colorClass: "text-[var(--color-rating-superhuman)]", fillClass: "bg-[var(--color-rating-superhuman)]", emoji: "🚀" };
  if (level > 6) return { label: "Excellent",      colorClass: "text-[var(--color-rating-excellent)]", fillClass: "bg-[var(--color-rating-excellent)]", emoji: "🎯" };
  if (level > 3) return { label: "Good",           colorClass: "text-[var(--color-rating-good)]", fillClass: "bg-[var(--color-rating-good)]", emoji: "👍" };
  if (level > 1) return { label: "Average",        colorClass: "text-[var(--color-rating-average)]", fillClass: "bg-[var(--color-rating-average)]", emoji: "🙂" };
...
export default function MemoryMatrix() {
  ...
  const playCellSound = useCallback((cell) => {
    ...
    playTone(audio, soundFrequencies[cell % soundFrequencies.length]);
  }, [startAudio]);

  const createSequence = useCallback((length) => {
    return Array.from({ length }, () => randBetween(0, gridCells.length - 1));
  }, [gridCells.length]);
  ...
  const resetGame = useCallback(() => {
    clearTimers();
    setLevel(1);
    const firstSeq = createSequence(1);
  ...
  const advanceLevel = useCallback(() => {
    clearTimers();
    const nextSequence = [...sequence, randBetween(0, gridCells.length - 1)];
  ...
  }, [clearTimers, gridCells.length, level, sequence]);

  const fetchLeaderboard = useCallback(async () => {
    setBoardLoading(true);
    try {
      const { data } = await api.get("/scores/memory-matrix/leaderboard");
  ...
  const fetchMyHistory = useCallback(async () => {
    ...
    try {
      const { data } = await api.get(`/scores/memory-matrix/my-history?limit=${MOVING_AVG_WINDOW}`);
  ...
  const completeRound = useCallback(() => {
    ...
    setHistory((prev) => [{ id: ++historyId.current, value: level }, ...prev].slice(0, MAX_HISTORY));
    if (user) saveScore(level);
    autoAdvanceRef.current = setTimeout(() => advanceLevel(), 1400);
  }, [advanceLevel, historyId, level, saveScore, user]);

  const handleTileClick = useCallback((cell) => {
    if (phase !== STATE.INPUT || inputIndex >= sequence.length) return;
    ...
    if (cell !== expected) {
      failRound(`Wrong tile! You reached level ${Math.max(0, level - 1)}.`);
      return;
    }

    const nextIndex = inputIndex + 1;
    setInputIndex(nextIndex);
    setMessage(nextIndex === sequence.length ? "Great! Sequence complete." : `Good: ${nextIndex} / ${sequence.length}`);

    if (nextIndex === sequence.length) {
      completeRound();
    }
  }, [completeRound, failRound, inputIndex, level, phase, playCellSound, sequence]);
  ...
  useEffect(() => {
    if (phase !== STATE.SHOWING || sequence.length === 0) return;
    ...
    const playStep = () => {
      ...
      showingRef.current = setTimeout(() => {
        setHighlightedCell(null);
        step += 1;
        showingRef.current = setTimeout(playStep, PATTERN_STEP_MS / 2);
      }, PATTERN_STEP_MS);
    };

    playStep();
    return () => clearTimeout(showingRef.current);
  }, [level, phase, playCellSound, sequence]);
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          <div className="rounded-3xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 shadow-2xl mb-10">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-xl font-semibold text-[var(--color-text)]">Memory Matrix</p>
                <p className="text-sm text-[var(--color-text-muted)] mt-1">
                  Repeat the sequence of glowing tiles. Each successful round adds one more.
                </p>
              </div>
          ...
          <div className="mt-10">
            <div className="mb-4 flex items-center justify-between gap-4">
              <div>
                <h2 className="text-[var(--color-text)] font-semibold text-lg">Memory Grid</h2>
                <p className="text-sm text-[var(--color-text-muted)]">Tap the tiles in the order you saw them.</p>
              </div>
            ...
            <div className={`grid gap-3 ${inputAllowed ? "" : "pointer-events-none blur-sm opacity-80"}`} style={{ gridTemplateColumns: `repeat(${gridSize}, minmax(0, 1fr))` }}>
              {gridCells.map((cell) => {
                const isHighlighted = highlightedCell === cell;
                return (
                  <button
                    key={cell}
                    type="button"
            ...
            {phase === STATE.FAIL && showCorrectOnFail && correctSequence.length > 0 && (
              <div className="mt-4 rounded-3xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
                <p className="text-sm text-[var(--color-text-muted)] mb-3">Correct sequence:</p>
                <div className="flex flex-wrap gap-2">
                  {correctSequence.map((cell, index) => (
